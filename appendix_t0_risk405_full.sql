...
-- 剔除前覆盖（结果4：可复现脚注中的无分数）
DROP TABLE IF EXISTS tmp_cov;
CREATE TEMP TABLE tmp_cov AS
SELECT
    TO_CHAR(DATE_TRUNC('month', t.vir_date), 'YYYY-MM') AS ym,
    COUNT(*) AS n_t0_raw,
    SUM(CASE WHEN m.serial_id IS NULL THEN 1 ELSE 0 END) AS n_no_join,
    SUM(CASE WHEN m.serial_id IS NOT NULL
              AND (m.kabybxgboost405apd7crecyc IS NULL OR m.kabybxgboost405apd7crecyc = -9999999)
             THEN 1 ELSE 0 END) AS n_miss,
    SUM(CASE WHEN m.kabybxgboost405apd7crecyc IS NOT NULL AND m.kabybxgboost405apd7crecyc <> -9999999
...
WHERE NOT EXISTS (
    ...
    WHERE m.serial_id = t.credit_serial_id
      AND m.kabybxgboost405apd7crecyc IS NOT NULL
      AND m.kabybxgboost405apd7crecyc <> -9999999
);
...
SELECT
    PERCENTILE_CONT(0.30) WITHIN GROUP (ORDER BY m.kabybxgboost405apd7crecyc) AS p30,
    PERCENTILE_CONT(0.60) WITHIN GROUP (ORDER BY m.kabybxgboost405apd7crecyc) AS p60
FROM tmp_m_origin t
INNER JOIN model_result_copy m ON m.serial_id = t.credit_serial_id
WHERE m.kabybxgboost405apd7crecyc IS NOT NULL
  AND m.kabybxgboost405apd7crecyc <> -9999999;
...
SELECT
    ...
    t.credit_serial_id,
    CASE
      WHEN m.kabybxgboost405apd7crecyc IS NULL THEN NULL
      WHEN m.kabybxgboost405apd7crecyc = -9999999 THEN NULL
      ELSE m.kabybxgboost405apd7crecyc
    END AS score,
    CASE
      WHEN m.kabybxgboost405apd7crecyc IS NULL OR m.kabybxgboost405apd7crecyc = -9999999 THEN '无分'
      WHEN m.kabybxgboost405apd7crecyc <= (SELECT p30 FROM tmp_cuts) THEN '较差'
      WHEN m.kabybxgboost405apd7crecyc <= (SELECT p60 FROM tmp_cuts) THEN '一般'
      ELSE '最好'
    END AS tier,
