#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""跑 appendix_t0_risk407_full.sql，写出 HTML 报告。"""
...
SQL_PATH = HERE / "appendix_t0_risk407_full.sql"
HTML_PATH = HERE / "T0获额风控分407三档当日提单息费逾期盈利报告.html"
...
def main():
    ...
    Path(HERE / "t0_risk407_monthly.json").write_text(
...
def write_html(sql_text, p30, p60, monthly, tier):
    def ym_star(ym):
        return f"{ym}*" if ym == "2026-06" else ym
...
<title>T0获额风控分407三档：当日提单息费逾期盈利</title>
...
<main class="wrap">
<header class="hero">
<div class="kicker">T0 · 结清0在贷 · 获额时点 KabyBXgboost407Apd7RePrime · 2026-05～08</div>
<h1>风控分三档：当天提单率、客群结构、息费、到期逾期与盈利</h1>
<div class="hero-meta">
分：<code>model_result_copy.serial_id</code> = 获额订单号，字段 kabybxgboost407apd7reprime；分越高资质越好（高分档到期逾期更低、盈利更高）<br>
分层：全量 T0 有分样本 P30={p30:.1f} / P60={p60:.1f}（较差 ≤P30，一般 P30–P60，最好 &gt;P60）<br>
息费统计当天提单订单（不卡放款）；盈利与逾期统计当天提单且已放款、已到期订单<br>
T0 用户日 {int(sum(r['n_t0'] for r in monthly)):,}（仅 5–8 月有分样本）。* 6 月剔除无 407 分获额单 5,549 笔，该月数字仅供参考。8 月到期未完全成熟，逾期/盈利仅供方向。
</div>
</header>

<h2>结论</h2>
<div class="callout"><div class="ch">1. 当天提单率下降，和息费抬升同时发生</div>
总体当天提单率 {pct(m0['apply_rate_pct'])} → {pct(m1['apply_rate_pct'])}（{d_apply:+.2f}pp）。当天提单订单加权息费 {rate_pct(m0['fee_rate'])} → {rate_pct(m1['fee_rate'])}（{d_fee:+.2f}pp）。价格上去、当天转化下来。主对比为 5 月 vs 8 月；6 月标 *，剔除无分单后仅供参考，不作拐点。</div>
...
5 月最好档接近一半，7–8 月较差档已超过最好档。这与「息费升 → 好资质少提、差资质占比升」同向，但不能单独当成因果鉴定。</div>
...
<article class="card"><div class="label">当天提单率 5月→8月</div><div class="num">{pct(m0['apply_rate_pct'])}→{pct(m1['apply_rate_pct'])}</div><div class="sub">{d_apply:+.2f}pp</div></article>
...
<h2>一、月度总体</h2>
<p class="desc">先看全部 T0，再看当天提单内部。提单率分母是当月全部获额用户日；息费是当天提单单（含未放款）；逾期与盈利是当天提单且已放款、已到期。标 * 的 6 月为剔除无分单后的参考值。</p>
<div class="grid g2">
...
<p class="desc">* 6 月：原 T0 获额 22,635 笔，其中 5,549 笔无 407 分已剔除，表内仅保留 17,086 笔有分样本，仅供参考。</p>
...
<li>模型分：获额订单号关联 model_result_copy.kabybxgboost407apd7reprime；5/7/8 月全样本有分；6 月剔除无分 5,549 笔后进入样本，该月标 * 仅供参考。</li>
...
<section id="sql">
<h2>附录：完整 PGSQL</h2>
<p class="desc">在 kaby_dw 执行；<code>SET search_path TO wangchuanliang, public;</code>。三个结果集依次为阈值、月度总体、月度×三档。同目录文件 <code>appendix_t0_risk407_full.sql</code>。默认收起，点击展开。</p>
<details>
<summary>完整 PGSQL（点击展开）</summary>
<pre class="sql">{sql_esc}</pre>
</details>
</section>
<p class="foot">数据：kaby_dw · wangchuanliang · 2026-05-01～2026-08-31 · 6 月无 407 分获额单已剔除（该月标 *） · 报告生成于分析当日库内 CURRENT_DATE</p>
</main>
<script>
const D = {payload};
