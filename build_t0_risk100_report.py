#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""跑 appendix_t0_risk100_full.sql，写出 HTML 报告。"""
...
SQL_PATH = HERE / "appendix_t0_risk100_full.sql"
HTML_PATH = HERE / "T0获额风控分100三档当日提单息费逾期盈利报告.html"
...
def main():
    ...
    Path(HERE / "t0_risk100_monthly.json").write_text(
        json.dumps({"p30": p30, "p60": p60, "monthly": monthly, "tier": tier}, ensure_ascii=False, indent=2),
        encoding="utf-8",
    )
    write_html(sql_text, p30, p60, monthly, tier)
    print("HTML", HTML_PATH)


def write_html(sql_text, p30, p60, monthly, tier):
    m0, m1 = monthly[0], monthly[-1]
    ym = [r["ym"] for r in monthly]
    by_tier = {"最好": [], "一般": [], "较差": []}
    for r in tier:
    ...
    def tier_table(name):
        ...
        for r in rows:
            body.append(
                "<tr>"
                f"<td>{r['ym']}</td><td>{int(r['n_t0']):,}</td><td>{int(r['n_same_day']):,}</td>"
...
<title>T0获额风控分100三档：当日提单息费逾期盈利</title>
...
<main class="wrap">
<header class="hero">
<div class="kicker">T0 · 结清0在贷 · 获额时点 KabyBXgboost100Apd7CashCreCyc · 2026-01～06</div>
<h1>风控分三档：当天提单率、客群结构、息费、到期逾期与盈利</h1>
<div class="hero-meta">
分：<code>model_result_copy.serial_id</code> = 获额订单号，字段 kabybxgboost100apd7cashcrecyc；分越高资质越好（高分档到期逾期更低、盈利更高）<br>
分层：全量 T0 有分样本 P30={p30:.1f} / P60={p60:.1f}（较差 ≤P30，一般 P30–P60，最好 &gt;P60）<br>
息费统计当天提单订单（不卡放款）；盈利与逾期统计当天提单且已放款、已到期订单<br>
T0 用户日 {int(sum(r['n_t0'] for r in monthly)):,}（已剔除 2 月无 100 分的 1 笔）。仅统计 2026-01～06。6 月到期未完全成熟，逾期/盈利仅供方向。
</div>
</header>

<h2>结论</h2>
<div class="callout"><div class="ch">1. 当天提单率下降，和息费抬升同时发生</div>
总体当天提单率 {pct(m0['apply_rate_pct'])} → {pct(m1['apply_rate_pct'])}（{d_apply:+.2f}pp）。当天提单订单加权息费 {rate_pct(m0['fee_rate'])} → {rate_pct(m1['fee_rate'])}（{d_fee:+.2f}pp）。价格上去、当天转化下来。</div>
<div class="callout red"><div class="ch">2. 获额客群没有变差，当天提单客群在变差</div>
T0 获额结构：最好档 {pct(m0['pct_t0_good'])} → {pct(m1['pct_t0_good'])}（{d_good_t0:+.2f}pp），较差档 {pct(m0['pct_t0_poor'])} → {pct(m1['pct_t0_poor'])}（{d_poor_t0:+.2f}pp）。3 月最好档一度到 42.94%，之后回落，6 月仍略好于 1 月。<br>
当天提单结构：最好档 {pct(m0['pct_apply_good'])} → {pct(m1['pct_apply_good'])}（{d_good_ap:+.2f}pp），较差档 {pct(m0['pct_apply_poor'])} → {pct(m1['pct_apply_poor'])}（{d_poor_ap:+.2f}pp）。3–4 月提单较差档曾降到约 36%–38%，5–6 月重新升到 45.64%、48.73%。问题在转化端，不在获额端。</div>
<div class="callout gold"><div class="ch">3. 最好档当天提单率掉得最多</div>
最好档 {pct(g0['apply_rate_pct'])} → {pct(g1['apply_rate_pct'])}；一般档 {pct(mid0['apply_rate_pct'])} → {pct(mid1['apply_rate_pct'])}；较差档 {pct(p0['apply_rate_pct'])} → {pct(p1['apply_rate_pct'])}。三档都在掉，最好档掉幅最大（约 −12pp），较差档仍是最高转化。总提单率下降，主要是好客更不提、差客继续提。</div>
...
总体到期盈利率 {rate_pct(m0['profit_rate'])} → {rate_pct(m1['profit_rate'])}（{d_pf:+.2f}pp），逾期 {pct(m0['overdue_pct'])} → {pct(m1['overdue_pct'])}（{d_ov:+.2f}pp）。6 月到期尚未完全成熟，逾期抬升含观察窗口因素。较差档 1–6 月到期盈利持续为负或接近 0。</div>
...
<article class="card"><div class="label">当天提单率 1月→6月</div><div class="num">{pct(m0['apply_rate_pct'])}→{pct(m1['apply_rate_pct'])}</div><div class="sub">{d_apply:+.2f}pp</div></article>
...
<h2>一、月度总体</h2>
<p class="desc">先看全部 T0，再看当天提单内部。提单率分母是当月全部获额用户日；息费是当天提单单（含未放款）；逾期与盈利是当天提单且已放款、已到期。</p>
<div class="grid g2">
<div class="chart"><h3>图1 当天提单率 vs 加权息费</h3><p>左轴提单率，右轴息费</p><div class="box"><canvas id="c1"></canvas></div></div>
<div class="chart"><h3>图2 到期逾期率 vs 到期盈利率</h3><p>仅已放款且 due_date 已过；6月到期偏少</p><div class="box"><canvas id="c2"></canvas></div></div>
</div>
...
<li>模型分：获额订单号关联 model_result_copy.kabybxgboost100apd7cashcrecyc；1–6 月剔除 2 月 1 笔无分后全样本有分。</li>
...
<section id="sql">
<h2>附录：完整 PGSQL</h2>
<p class="desc">在 kaby_dw 执行；<code>SET search_path TO wangchuanliang, public;</code>。三个结果集依次为阈值、月度总体、月度×三档。同目录文件 <code>appendix_t0_risk100_full.sql</code>。默认收起，点击展开。</p>
<details>
<summary>完整 PGSQL（点击展开）</summary>
<pre class="sql">{sql_esc}</pre>
</details>
</section>
<p class="foot">数据：kaby_dw · wangchuanliang · 2026-01-01～2026-06-30 · 已剔除无 100 分的 1 笔 · 报告生成于分析当日库内 CURRENT_DATE</p>
</main>
<script>
const D = {payload};
