...
SELECT
    PERCENTILE_CONT(0.30) WITHIN GROUP (ORDER BY m.kabybxgboost363dpd7cre) AS p30,
    PERCENTILE_CONT(0.60) WITHIN GROUP (ORDER BY m.kabybxgboost363dpd7cre) AS p60
FROM tmp_m_origin t
...
SELECT
    ...
    t.credit_serial_id,
    CASE
      WHEN m.kabybxgboost363dpd7cre IS NULL THEN NULL
      WHEN m.kabybxgboost363dpd7cre = -9999999 THEN NULL
      ELSE m.kabybxgboost363dpd7cre
    END AS score,
    CASE
      WHEN m.kabybxgboost363dpd7cre IS NULL OR m.kabybxgboost363dpd7cre = -9999999 THEN '无分'
      WHEN m.kabybxgboost363dpd7cre <= (SELECT p30 FROM tmp_cuts) THEN '较差'
      WHEN m.kabybxgboost363dpd7cre <= (SELECT p60 FROM tmp_cuts) THEN '一般'
      ELSE '最好'
    END AS tier,
