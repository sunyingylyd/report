#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""跑 appendix_t0_risk363_full.sql，写出 HTML 报告。"""
...
SQL_PATH = HERE / "appendix_t0_risk363_full.sql"
HTML_PATH = HERE / "T0获额风控分363三档当日提单息费逾期盈利报告.html"
...
def main():
    ...
    Path(HERE / "t0_risk363_monthly.json").write_text(
...
<title>T0获额风控分363三档：当日提单息费逾期盈利</title>
...
<main class="wrap">
<header class="hero">
<div class="kicker">T0 · 结清0在贷 · 获额时点 KabyBXGBoost363Dpd7Cre · 2026-01～08</div>
<h1>风控分三档：当天提单率、客群结构、息费、到期逾期与盈利</h1>
<div class="hero-meta">
分：<code>model_result_copy.serial_id</code> = 获额订单号，字段 kabybxgboost363dpd7cre；分越高资质越好（已用到期 APD7 校验）<br>
分层：全量 T0 有分样本 P30={p30:.1f} / P60={p60:.1f}（较差 ≤P30，一般 P30–P60，最好 &gt;P60）<br>
息费统计当天提单订单（不卡放款）；盈利与逾期统计当天提单且已放款、已到期订单<br>
T0 用户日 {int(sum(r['n_t0'] for r in monthly)):,}；获额单 363 分覆盖率 100%。8 月到期未成熟，逾期/盈利仅供方向。
</div>
</header>
...
最好档 {pct(g0['apply_rate_pct'])} → {pct(g1['apply_rate_pct'])}；一般档 {pct(mid0['apply_rate_pct'])} → {pct(mid1['apply_rate_pct'])}；较差档 {pct(p0['apply_rate_pct'])} → {pct(p1['apply_rate_pct'])}。总提单率下降是各档转化都弱，叠加获额结构变差。</div>
<div class="callout green"><div class="ch">4. 最好档加价后仍更能覆盖风险；较差档息费最高、逾期最高、盈利最薄</div>
...
总体到期盈利率 {rate_pct(m0['profit_rate'])} → {rate_pct(m1['profit_rate'])}（{d_pf:+.2f}pp），逾期 {pct(m0['overdue_pct'])} → {pct(m1['overdue_pct'])}（{d_ov:+.2f}pp）。息费升没有稳定翻译成更高到期盈利，结构变差会把加价吃掉。</div>
<div class="callout"><div class="ch">经营含义</div>
不宜再用降息/提额去抬 T0 当天提单率——当天单盈利薄，较差档占比还在升。更合理的是：保护最好档定价与转化；较差档控制让利；把经营重心放到结清当天之后仍未提单的时段。</div>
...
<article class="card"><div class="label">T0 获额档占比</div><div class="num">{pct(m0['pct_t0_poor'])}→{pct(m1['pct_t0_poor'])}</div><div class="sub">{d_poor_t0:+.2f}pp</div></article>
...
<li>模型分：获额订单号关联 model_result_copy.kabybxgboost363dpd7cre；本窗口 177,133 笔全部有分。</li>
<li>分越高逾期越低（到期 APD7：低分档 42.7%，高分档 25.1%）。</li>
<li>当天提单不卡 is_due / is_remit。息费不卡放款。盈利、逾期要求 is_remit=1 且已到期。</li>
</ul>

<section id="sql">
<h2>附录：完整 PGSQL</h2>
<p class="desc">在 kaby_dw 执行；<code>SET search_path TO wangchuanliang, public;</code>。三个结果集依次为阈值、月度总体、月度×三档。同目录文件 <code>appendix_t0_risk363_full.sql</code>。默认收起，点击展开。</p>
<details>
<summary>完整 PGSQL（点击展开）</summary>
<pre class="sql">{sql_esc}</pre>
</details>
</section>
<p class="foot">数据：kaby_dw · wangchuanliang · 2026-01-01～2026-08-31 · 报告生成于分析当日库内 CURRENT_DATE</p>
</main>
<script>
const D = {payload};
